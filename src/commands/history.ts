...
export async function execute(interaction: CommandInteraction) {
  ...
  .then(async (historyList) => {
    ...
    for (var historyItem of historyList) {
      ...
      } else if (historyItem.PointsRecieved === "1") {
        // TODO YELLOW CURRENTLY GREEN
        ctx.fillStyle = "#64e3a1";
      } else if (historyItem.PointsRecieved === "3") {
        // green
        ctx.fillStyle = "#64e3a1";
      }
      ...
      // score
      ctx.fillText(`${historyItem.Team1Score} - ${historyItem.Team2Score}`, 455, offset)
